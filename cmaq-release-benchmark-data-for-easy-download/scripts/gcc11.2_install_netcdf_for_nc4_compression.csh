...
#  -------------------
   ...
   mkdir -p $cwd/LIBRARIES_gcc11.2
   setenv INSTDIR $cwd/LIBRARIES_gcc11.2
...
#  make check > make.gcc11.check
   make install |& tee make.gcc11.log
#  ---------------------------
...
#  ---------------------------------
   ...
   setenv LIBS "-L${INSTDIR}/lib -lnetcdf -lhdf5_hl -lhdf5 -lhdf5_fortran -lm -ldl -lz -lcurl"
...
# test version that has been installed
   cd $INSTDIR/bin
   ls h5diff
   whereis h5diff 
   nc-config --version
...
## If you do not have access to openmpi using environment variables, you can install it using the following instuctions that are currently commented out.
#  --------------------------
